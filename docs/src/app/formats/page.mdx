...
agent-xlsx includes automatic date detection. Excel stores dates as serial numbers (e.g. `45658` for 2024-12-31). The Polars adapter detects columns that contain date serial numbers and converts them to ISO 8601 strings (`YYYY-MM-DD` or `YYYY-MM-DDTHH:MM:SS`) in the output.

Date detection is applied automatically during `read` commands. This is always-on best-effort — columns with date serial numbers are converted; those that don't match are left as-is.
